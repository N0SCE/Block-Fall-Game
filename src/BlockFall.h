...
public:
    ...
    void ImplementVector(std::string line, vector<vector<bool>> &vector);
    void SetRotations(Block* block);
    void RotateVector(Block* vectorToRotate, vector<vector<bool>> &vector);
    void initialize_next_rotations(Block* block);
};
